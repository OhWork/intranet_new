...
if (!isset($_SESSION['RF']['ascending']))
{
	$_SESSION['RF']['ascending'] = TRUE;
}
...
if($ftp){
	try{
		$files2 = $ftp->scanDir($ftp_base_folder.$upload_dir.$rfm_subfolder.$subdir);
...
<!-- header div end -->
	...
	<li class="pull-left">
		...
		<li><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"ascending":"descending"; } ?>" href="javascript:void('')" data-sort="name"><?php echo trans('Filename');?></a></li>
		<li><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"ascending":"descending"; } ?>" href="javascript:void('')" data-sort="date"><?php echo trans('Date');?></a></li>
		<li><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"ascending":"descending"; } ?>" href="javascript:void('')" data-sort="size"><?php echo trans('Size');?></a></li>
		<li><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"ascending":"descending"; } ?>" href="javascript:void('')" data-sort="extension"><?php echo trans('Type');?></a></li>
		</ul>
		</div>
	</li>
	...
	<div class="span12">
		...
		?>
		<h2><b><?php echo $cutrsshowsubfol;?></b></h>
		...
		<div class='img-dimension'><?php echo "Download";?></div>
				...
				<figure data-name="<?php $result['files_name'];?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
					...
					<div class="img-precontainer-mini directory" style="margin-top :30px;">
						...
						<div class="box">
							<?php
						    $folders = [];
						    $folder_id = $result['folder_position'];
						    while ($folder_id != null) {
						        $selectelse = $db->findByPK('folder','folder_id', $folder_id)->executeAssoc();
						        array_push($folders, $selectelse['folder_name']);
						        $folder_id = $selectelse['folder_position'];
						    }
                        ?>
							<a href="source<?php
		...
		<li>
			...
			<figure class="<?php if($file=="..") echo "back-";?>directory" data-name="<?php echo $result['folder_name'];?>"  data-type="<?php if($file!=".."){ echo "dir"; } ?>">
					...
					<div class="box">
                        ...
                        ?>
						<a class="folder-link" href="admin_index.php?url=fm_dialog.php&?editor=0&type=0&lang=en_EN&popup=0&crossdomain=0&field_id=&relative_url=0&akey=key&fldr=
									<?php if($result['folder_name'] != ''){
										 for($i = sizeof($folders); $i >= 0; $i--) {
								         echo $folders[$i].'/';
								     }
			...
			<figure class="<?php if($file=="..") echo "back-";?>directory" data-name="<?php echo $result['folder_name'];?>"  data-type="<?php if($file!=".."){ echo "dir"; } ?>">
					...
					<div class="box">
						...
						<?php }else{
							$query2 = $db->findByPK("folder","folder_id",$result['folder_position'])->executeAssoc();
							?>
							<a class="folder-link" href="index.php?url=fm_dialog.php&?editor=0&type=0&lang=en_EN&popup=0&crossdomain=0&field_id=&relative_url=0&akey=key&fldr=
									<?php if($result['folder_name'] != ''){
										 for($i = sizeof($folders); $i >= 0; $i--) {
								         echo $folders[$i].'/';
								     }
...
?>
			...
			?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<?php
						...
						$rs = $sql2['files_id'];
						$sql3 = $db->findByPK2('cdl','files','files_files_id',"'$rs'")->executeAssoc();
				?>
				<input type="hidden" class="date" value="<?php echo $file_array['date'];?>"/>
				<input type="hidden" class="size" value="<?php echo $file_array['size'] ?>"/>
...
<!-- 				<div class='file-extension'><?php echo $file_array['extension'];?></div> -->
				<figcaption>
					...
					title="<?php echo trans('Preview')?>" data-url="fm_ajax_calls.php?action=media_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					...
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="fm_ajax_calls.php?action=cad_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php }elseif($preview_text_files && in_array($file_array['extension'],$previewable_text_file_exts)){ ?>
					<?php if($file_array['extension'] == 'pdf'){?>
					<a class="tip-right" title="<?php echo trans('Preview')?>" href="source/<?php echo $subdir .$filename;?>.pdf"><i class=" icon-eye-open"></i></a>
					<?php }else{?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="fm_ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php } ?>
					<?php }elseif($googledoc_enabled && in_array($file_array['extension'],$googledoc_file_exts)){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="fm_ajax_calls.php?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
