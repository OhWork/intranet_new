...
<?php
    ...
    $columns = array('eventzlfot_code','zlfot_nameth','zlfot_tel','zlfot_datestart','zoo_name');
    $row = "<div class='row'>";
    $rowend = "</div>";
    $form = new form();
    $labelsearchipzpo = new label('ค้นหา');

            $rs = $db->findByPK33DESC('eventzlfot','user','zoo','zlfot.user_user_id','user.user_id','user.subzoo_zoo_zoo_id','zoo.zoo_id','zlfot_status','"C"','zlfot_id')->execute();  ?> 
