...
//     $txtnocompletedetail = new textarea('problem_detailwaitcomplete','form-control','','','','','');
    ...
    if(!empty($_GET['id'])){
		$id = $_GET['id'];
		$r = $db->findByPK44('eventconfer','conferroom','zoo','headncf','conferroom_conferroom_id','confer_id','zoo_zoo_id','zoo_id','headncf_headncf_id','headncf_id','eventconfer_id',$id)->executeRow();
		 $year = date("Y")+543;
          $md = date("m-d");
          $time = date("H:i");
