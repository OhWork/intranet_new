...
// 	$sql ="SELECT * FROM eventconfer WHERE eventconfer_start >= '$datestart' AND  eventconfer_end <= '$dateend'";
	$sql = "SELECT * FROM eventconfer WHERE ((eventconfer_start BETWEEN '$datestart' AND '$dateend') OR (eventconfer_end BETWEEN '$datestart' AND '$dateend'))	 AND confer_conferroom_id = '$conferroom_id' AND eventconfer_status  = 'Y'";
