...
}elseif (is_file($file_path) && is_readable($file_path))
{
	...
	$rsselectid = $db->findByPK('files','files_name',"'$name'")->executeAssoc();
	$rsselectfiles_id = $db->findByPK22('cdl','files','files_files_id','files_id','files_name',"'$name'")->executeAssoc();

if(empty($rsselectfiles_id['files_files_id'])){
	$rs = $db->insert('cdl',array(
					'cdl_count' => 1,
					'files_files_id' => $rsselectid['files_id'],
				));

}
else{
	$data['cdl_count'] = $rsselectfiles_id['cdl_count']+1;
	$rsfix = $db->update('cdl',$data,'files_files_id',$rsselectfiles_id['files_files_id']);
}
